...
namespace Lentinula.Api.Services;
...
public class UserService : IUserService
{
    ...
    {
        if (await _dbContext.Users.AnyAsync(u => u.Account == dto.Account, cancellationToken))
        {
            throw new BasicException(ResponseCode.Fail, "该账号已被注册");
        }
        if (await _dbContext.Users.AnyAsync(u => u.NickName == dto.NickName, cancellationToken))
        {
    ...
    {
        var user = await _dbContext.Users.FirstOrDefaultAsync(x => x.Account == dto.Account, cancellationToken);
